...
function isRecentlySeen(lastSeen) {
  if (!(lastSeen instanceof Timestamp)) return false;
  const nowMs = Date.now();
  const lastSeenMs = lastSeen.toMillis();
  return nowMs - lastSeenMs <= ONLINE_STALE_MINUTES * 60 * 1000;
}
...
export async function getOnlineUsersCount() {
  try {
    ...
    return snapshot.docs.filter((doc) => isRecentlySeen(doc.data()?.lastSeen)).length;
  } catch (error) {
...
export async function getRealtimeUsers() {
  try {
    ...
    const users = [];
    snapshot.forEach((doc) => {
      if (isRecentlySeen(doc.data()?.lastSeen)) {
        users.push({
          userId: doc.id,
          ...doc.data(),
        });
